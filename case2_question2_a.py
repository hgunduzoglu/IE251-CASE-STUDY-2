# e-constraint method solution. In this part total cost is constrained.
import case2_data 
...
def total_cost_epsilon_constraint_rule(md1):
    e = 67296.875   # epsilon value for total cost ((worst scenerio - best scenerio) / 4)
    minimum_total_cost = 2455450           # best scenerio (the minimum) for total cost
    #return md1.total_cost ==  minimum_total_cost + 0*e
    #return md1.total_cost == minimum_total_cost + 1*e
    #return md1.total_cost == minimum_total_cost + 2*e
    #return md1.total_cost == minimum_total_cost + 3*e
    return md1.total_cost == minimum_total_cost + 4*e
md1.total_cost_epsilon_constraint = pyo.Constraint(rule = total_cost_epsilon_constraint_rule)

def idle_objective_rule(md1):
    return sum(md1.IW[w,t] for w in md1.W for t in md1.T)
md1.idle_objective = pyo.Objective(rule = idle_objective_rule, sense= pyo.minimize, doc="minimum number of idle workers")
...
md1.pprint()
md1.idle_objective.display() # show value of objective function in console
md1.total_cost.display()
