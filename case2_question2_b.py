# e-constraint method solution. In this part total number of workers remained idle is constrained.
import case2_data 
...
def total_idle_rule(md1):
    return sum(md1.IW[w,t] for w in md1.W for t in md1.T)
md1.total_idle = pyo.Expression(rule = total_idle_rule, doc="total number of idle workers")

def total_idle_epsilon_constraint_rule(md1):
    e = 75  # epsilon value for total idle workers. ((worst scenerio - best scenerio ) /4)
    minimum_total_idle = 2871    # best scenerio ( minimum) for total idle workers
    #return md1.total_idle ==  minimum_total_idle + 0*e
    #return md1.total_idle == minimum_total_idle + 1*e
    #return md1.total_idle == minimum_total_idle + 2*e
    return md1.total_idle == minimum_total_idle + 3*e
    #return md1.total_idle == minimum_total_idle + 4*e
md1.total_idle_epsilon_constraint = pyo.Constraint(rule = total_idle_epsilon_constraint_rule)
...
md1.total_cost = pyo.Objective(rule = total_cost_rule , sense= pyo.minimize, doc="Total minimum cost")
